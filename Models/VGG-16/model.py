...
class VGG16Gender(nn.Module):
    """
    VGG16 implementation for gender classification with batch normalization.
    ...
    def __init__(self,
        ...
        # Adaptive average pooling (similar to EfficientNet implementation)
        self.avgpool = nn.AdaptiveAvgPool2d((7, 7))

        # Classification layers (following EfficientNet structure with dropout)
        ...
        # Initialize weights (similar to EfficientNet implementation)
        if init_weights:
    ...
    def forward(self, x):
        ...
        Following the same pattern as EfficientNet implementation.
        """
        # Extract features
        ...
        # Flatten for classifier (similar to EfficientNet)
    ...
    def _initialize_weights(self):
        """
        Initialize weights following the same pattern as EfficientNet implementation.
        """
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                # Kaiming normal initialization (same as EfficientNet)
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                if m.bias is not None:
                    nn.init.constant_(m.bias, 0)
            elif isinstance(m, nn.BatchNorm2d):
                # Constant initialization (same as EfficientNet)
                nn.init.constant_(m.weight, 1)
                nn.init.constant_(m.bias, 0)
            elif isinstance(m, nn.Linear):
                # Normal initialization for linear layers (same as EfficientNet)
